...
```{r trans-conv, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=figs("trans-conv", "Conventional money transactions are completely one-dimensional.")}
...
An interesting idea might be the concept of "liquid democracy". XXX Add reference to liquid democracy
...
of the product etc. Maybe the color database can incorporate user ratings as well,
e.g. rating the usefulness or the quality of the product in use.
...
increases the threshold to add new products and introduces hierarchy. Any better
...
[@BitcoinEnergy], not for computing anything valuable, but only for the
validation of transactions. Another downside is that the system increases
...
that, there is even a dangerous tendency towards mining monopolies or oligarchies,
where big mining farms buy smaller competitors until only few (or one) very large
players are left.

<h4>Proof-of-stake</h4>

A variation of the proof-of-work algorithm is called *proof-of-stake*, in which
the expensive computation is omitted. The validator of the next block is selected
randomly, however favoring the nodes
