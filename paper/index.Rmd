...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=figs("transactions-conventional", "Conventional money transactions are completely one-dimensional.")}
...
caused by the dominance of money. In this sense, colorized money might add a
...
3. The cryptocurrency itself (a distributed ledger storing the transactions),
   for example as a blockchain in a framework like Ethereum.
...
e.g. rating the usefulness or the quality of the prduct in use.
...
rather crude implementation of the new cryptocurrency:
...
The key function is the still unimplemented `getColor()`. The color of the
transaction is an information that must be fetched from the outside non-digital
world. In smart contract jargon, a provider of such "external information" is
called an *oracle*, e.g. see http://www.oraclize.it/. It is such an oracle that
needs to be built in form of the collaborative platform where the colors are
determined, stored in the distributed color blockchain, which would be queried
by the function `getColor()` to fetch the currently valid color value of the
product.
